...
if (typeof CHARITY_IS_HOPE_STORAGE == "undefined")
  var CHARITY_IS_HOPE_STORAGE = {};

// Get global variable
function charity_is_hope_storage_get(var_name) {
  return charity_is_hope_isset(CHARITY_IS_HOPE_STORAGE[var_name])
    ? CHARITY_IS_HOPE_STORAGE[var_name]
    : "";
}
...
function charity_is_hope_storage_get_array(var_name, key) {
  return charity_is_hope_isset(CHARITY_IS_HOPE_STORAGE[var_name][key])
    ? CHARITY_IS_HOPE_STORAGE[var_name][key]
    : "";
}

// Set global array element
function charity_is_hope_storage_set_array(var_name, key, value) {
  if (!charity_is_hope_isset(CHARITY_IS_HOPE_STORAGE[var_name]))
    CHARITY_IS_HOPE_STORAGE[var_name] = {};
...
function charity_is_hope_empty(obj) {
  return (
    ...
    (typeof obj == "string" && charity_is_hope_alltrim(obj) == "") ||
    obj === 0
  );
}
    ...
    case "object":
      if (mixed_val == null) return "N;";
        ...
        for (var i = 0; i < mixed_val.length; i++) {
          ...
          if (ser) map.push(charity_is_hope_serialize(idxobj.idx) + ser);
        }
        ...
        for (var prop in mixed_val) {
          ...
          if (ser) props.push(charity_is_hope_serialize(prop) + ser);
        }
        if (obj_to_array)
...
function charity_is_hope_get_date_difference(dt1) {
  ...
  } else var a2 = charity_is_hope_parse_date(dt2);
      ...
      if (thead.hasClass("more_colors")) {
        ...
        while (rgb[0] < 0xf || rgb[1] < 0xf || rgb[2] < 0xf) {
          ...
          out +=
            ...
            charity_is_hope_components2hex(
              rgb[0] * 16 + rgb[0],
              rgb[1] * 16 + rgb[1],
              rgb[2] * 16 + rgb[2]
            ) +
...
function charity_is_hope_color_picker_show(id, fld, func) {
  ...
  var def =
    fld.val().substr(0, 1) == "#"
      ? fld.val()
      : charity_is_hope_rgb2hex(fld.css("backgroundColor"));
